<template>
  <div>
    <h3>
      ...
      <i v-if="expanded == 'training'" class="fas fa-question-circle" @click="toggleShowHelp()" />
    </h3>
    <div v-if="expanded == 'training'">
      <Help v-if="showHelp" />
      <table>
        <thead>
          <tr>
...
export default {
  ...
  methods: {
    toggleExpanded() {
      const expanded = this.expanded == 'training' ? '' : 'training'
