<template>
  <vue-final-modal name="contact-person" classes="modal-container" content-class="vfm__content modal-content" v-model="modals['contact-person']">
...
export default {
  ...
  methods: {
    hide() {
      this.$store.dispatch('hideModal', 'contact-person')
    },
    sendContact() {
      ...
      mailFuns.post({
        ...
        tomail: this.person.email,
        fromsite: site
        },
        'Thanks for your contacting {{ person.name }} - we\'ll be back in touch shortly'
      )
      this.hide()
    }
  }
